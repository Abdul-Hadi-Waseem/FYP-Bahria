"""
This module provides audio signal processing functions for feature extraction.
It includes time-domain and frequency-domain feature calculations used in audio analysis.
The code is based on pyAudioAnalysis library with modifications.
"""
...
def chromagram(signal, sampling_rate, window, step, plot=False, show_progress=False):
    """
    Computes and returns the chromagram (pitch class profile) of an audio signal.
    A chromagram represents the distribution of energy across the 12 pitch classes
    (musical notes A through G#) over time.
    
    Process:
    1. Normalizes input signal
    2. Segments signal into overlapping windows
    3. Computes FFT for each window
    4. Maps FFT bins to pitch classes
    5. Optionally visualizes the chromagram
    
    Args:
        signal: Input audio signal array
        sampling_rate: Sampling frequency in Hz
        window: Analysis window size in samples
        step: Number of samples between successive windows
        plot: Whether to display the chromagram plot
        show_progress: Whether to show a progress bar
    
    Returns:
        tuple: (chromagram matrix, time axis values, frequency axis labels)
    """
    window = int(window)
    step = int(step)
...
def spectrogram(signal, sampling_rate, window, step, plot=False, show_progress=False):
    ...
    Process:
    1. Segments signal into overlapping windows
    2. Computes FFT for each window
    3. Normalizes FFT magnitudes
    4. Optionally visualizes the spectrogram
    ...
    Args:
        ...
        window: Analysis window size in samples
        step: Number of samples between successive windows
        plot: Whether to display the spectrogram plot
        show_progress: Whether to show a progress bar
    
    Returns:
        tuple: (spectrogram matrix, time axis values, frequency axis values)
    """
    window = int(window)
    step = int(step)
    # signal = np.double(signal)
...
def feature_extraction(signal, sampling_rate, window, step, deltas=True):
    """
    Comprehensive audio feature extraction function that computes multiple features
    for each window of the signal. Features include:
    ...
    Optional delta features (differences between consecutive frames)
    
    Process:
    1. Segments signal into windows
    2. Computes FFT for each window
    3. Extracts all features from each window
    4. Optionally computes delta features
    5. Concatenates all features into a matrix
    ...
    Args:
        ...
        window: Analysis window size in samples
        step: Number of samples between successive windows
        deltas: Whether to compute delta features
        
    Returns:
        tuple: (feature matrix, list of feature names)
    """
