...
def Preprocessing(dir, preprocessed_dir_savesamples):
    ...
    # Make dir
    if os.path.exists(preprocessed_dir_savesamples) and Config.save_for_preprocessing_and_feature_extraction:
        shutil.rmtree(preprocessed_dir_savesamples)
    os.makedirs(preprocessed_dir_savesamples, exist_ok=True)
    ...
    for file_name in os.listdir(dir):
        ...
        # Update these lines
        data_dir = os.path.join(dir, file_name)
        label_dir = os.path.join(dir, f"{os.path.splitext(file_name)[0]}.txt")
        ...
        if not samples["signal"]:
            print(f"No samples found for {file_name}. Skipping...")
            continue
        ...
        if Config.save_for_preprocessing_and_feature_extraction:
            for i, (signal, label) in enumerate(zip(samples["signal"], samples["label"])):
                save_dir = os.path.join(preprocessed_dir_savesamples, f"{os.path.splitext(file_name)[0]}_{i}.dat")
